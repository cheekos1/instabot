...
class ImageProcessor {
  ...
  async createInstagramCard(imageUrl, avatarUrl, username, likeCount, imageIndex = 1, totalImages = 1) {
    try {
      // Optimized card dimensions - smaller for faster processing
      const cardWidth = 450; // Reduced from 500
      const headerHeight = 60; // Reduced from 70
      const likesAreaHeight = 25; // Reduced from 30
      const footerHeight = 25; // Reduced from 30
      const cardHeight = 400 + headerHeight + likesAreaHeight + footerHeight; // Smaller image - no quotes area
      const imageHeight = 400; // Reduced from 580 for faster processing
      const imageWidth = cardWidth - 20; // 430px wide image
      const avatarSize = 40; // Slightly smaller avatar
      const padding = 10;
