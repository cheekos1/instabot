...
class ImageProcessor {
  ...
  async createRoundedRectangleImage(imageUrl, width, height, borderRadius = 20) {
    try {
      ...
      // If image processing fails, return a fallback that indicates to use raw URL
      console.log('🔄 Image processing failed, falling back to raw URL display');
      return { 
        isGif: false, 
  ...
  async downloadImage(url) {
    try {
      ...
      if (!response.ok) {
        console.error(`❌ HTTP Error ${response.status}: ${response.statusText} for URL:`, url.substring(0, 50) + '...');
      ...
      if (!contentType || !contentType.startsWith('image/')) {
        console.error(`❌ Invalid content type: ${contentType || 'unknown'} for URL:`, url.substring(0, 50) + '...');
      ...
      if (!buffer || buffer.length === 0) {
        console.error(`❌ Empty buffer for URL:`, url.substring(0, 50) + '...');
        throw new Error('Downloaded buffer is empty or null');
      }
      
      console.log(`✅ Successfully downloaded ${buffer.length} bytes for:`, url.substring(0, 50) + '...');
      ...
      // Clear any potentially corrupted cache entry
  ...
  async createInstagramCard(imageUrl, avatarUrl, username, likeCount, imageIndex = 1, totalImages = 1) {
    try {
      ...
      // Check if this is a GIF or fallback - return flag to use raw image
      if (processedMainImage.isGif || processedMainImage.fallback) {
        const reason = processedMainImage.isGif ? 'GIF animation preserved' : 'Image processing failed';
        console.log(`🎬 ${reason} - returning flag to use raw image instead of processed card`);
        // For GIFs or failed processing, we return a special object indicating to use the raw image
        return {
          isGif: processedMainImage.isGif,
          originalUrl: imageUrl,
          fallback: processedMainImage.fallback || false,
          message: `${reason} - showing original file`,
          error: processedMainImage.error
        };
      }
