...
module.exports = {
  ...
  async showImageCard(interaction, userId, imageIndex, userImages, user, targetUser, userQuotes = []) {
    try {
      ...
      // Check if this is a GIF - handle differently
      let cardAttachment;
      if (cardResult.isGif) {
        console.log('🎬 Using raw GIF for display');
        // For GIFs, we don't create a card attachment - we'll use the embed image instead
        cardAttachment = null;
      } else {
      ...
      if (cardResult.isGif) {
        // For GIFs, show the raw animated GIF directly
